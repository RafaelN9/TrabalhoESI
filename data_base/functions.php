<?php
    ...
    if(isset($_POST["login"]) && $_POST["login"] == "Entrar"){
        if(isset($_POST["loginEmail"]) && isset($_POST["loginPwd"])){
            $queryAluno = "SELECT Email FROM aluno WHERE Email = '$_POST[loginEmail]' AND Senha = '$_POST[loginPwd]' LIMIT 1";
            ...
            if($rowAluno = mysqli_fetch_assoc($resultAluno)){
                ...
                $queryCCP = "SELECT Email FROM professor INNER JOIN ccp on professor.CPF = ccp.CPF_Prof WHERE Email = '$_POST[loginEmail]' AND Senha = '$_POST[loginPwd]' LIMIT 1";
                $resultCCP = runSQL($queryCCP);
                if($rowCCP = mysqli_fetch_assoc($resultCCP)){
                    $_SESSION["tipo_usuario"] = 'ccp';
                    header("Location: http://localhost/trabalhoESI/view/index.php");
                }else{
                    $queryProfessor = "SELECT Email FROM Professor WHERE Email = '$_POST[loginEmail]' AND Senha = '$_POST[loginPwd]' LIMIT 1";
